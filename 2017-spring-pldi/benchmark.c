#include <stdio.h>
#include <stdlib.h>
typedef int I;typedef void V;typedef size_t B;typedef char C;
typedef struct cn{B i;I t;B c;struct cn*k[];}*CN;
#define R return
#define P printf
...
V help(V){P("benchmark <cpu|gpu> [<depth> <width>]\n");exit(1);}
...
V cpu(B d,B w){pp(d,w,"CPU");P("Before\n");CN t=mct(d,w);pct(t,1);
 P("After\n");t=fltc(t);pct(t,1);fct(t);}

V gpu(B d,B w){pp(d,w,"GPU");}

I main(I c,C*v[]){B d=3,w=2;if(c!=2&&c!=4)help();
 if(c==4){d=atoi(v[2]);w=atoi(v[3]);}
 if(!strcmp(v[1],"cpu"))cpu(d,w);else if(!strcmp(v[1],"gpu"))gpu(d,w);else help();
 R 0;}
